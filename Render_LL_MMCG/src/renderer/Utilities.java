...
/* La classe renderer.Utilities contiene le variabili già
 *  precalcolate e i metodi necessari per le intersezioni
 * La classe è gestita come un oggetto e con variabili non
 *  statiche poichè è necessario gestire tutte le intersezioni
 *  in modo interno all'oggetto durante il multithreading
 *  per evitare accessi non autorizzati ad altre variabili.
 *
 * Il principale problema presente è l'utilizzo di questa
 *  classe come oggetto obbligatorio per la classe Renderer
 *  per far si che, durante il multithreading, non si generino
 *  errori nel richiamo delle variabili inters e intersObj.
 * Sarebbe possibile riscrivere la classe in modo tale che
 *  restituisca ogni volta che necessario un vettore con la
 *  distanza e l'oggetto, in modo tale da non salvare variabili
 *  in questa classe e utilizzarla solo per gli strumenti utili
 *  al rendering
 */
...
public class Utilities {
	...
	public static final float EPSILON = 1.e-2f;
	public static final float MATH_PI = 3.14159265358979323846f;
	//1 su pi greco
	public static final float MATH_1_DIV_PI = 0.318309886183790671538f;
  ...
  Ray[] refract(Ray[] rays, Point3D i, Point3D n, Point3D ior) {
    ...
    //se l'angolo di incidenza è superiore a pi/2
    // allora devo cambiare il verso della normale e cambiare indice di rifrazione (prendendo il suo inverso)
    if(cos_theta_i<0.0f){
      cos_theta_i=-cos_theta_i;
...
//si verifica che il box index non sia vuoto
    if(Tree[index-1].nph != 0){
      ...
      // in tal caso l'indice corrisponde ad un box all'estremità dell'albero
      if((2*index)+1 < RenderAction.power){
        ...
        for(int i=0;i<n;i++){
          ...
          if(objX.t != null){
            ...
            //in realtà per evitare fenomeni di aliasing su
            // le facce piccole si è costretti a considerare un parametro che
